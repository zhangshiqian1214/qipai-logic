...
function M:get_hu_info(hand_cards)
    ...
    -- 检查东南西北中发白
    local success, eye = self:check_zi(cards)
    if not success then
        return false
    end

    -- 检查万
    success, eye = self:check_color(cards, 1, eye)
    if not success then
        return false
    end
    
    -- 检查筒
    success, eye = self:check_color(cards, 10, eye)
    if not success then
        return false
    end
    
    -- 检查条
    success = self:check_color(cards, 19, eye)
    return success
end
...
function M:check_color(cards, from, eye)
    ...
    elseif yu == 2 then
        ...
        eye = true
        -- 调用带将的拆分
        return self:split_eye(cards, from), eye
    end
    -- 调用不带将的拆分
    return self:split(cards, from, from+8), eye
end

function M:split_eye(cards, from)
    -- 找出本花色所有可能的将
    local eye_tbl = self:find_eye(cards, from, from+8)
    if not eye_tbl then
        return false
    end
    ...
    for i, _ in pairs(eye_tbl.eyes) do
        ...
        -- 拆分剩余牌
        if self:split(tmp_cards, 1, index) then
            return true
        end
        -- 把扣除的将加回来
        cards[i] = cards[i] + 2
    ...
    for i=from,to do
        ...
        if c == 0 or i == to then
            ...
            elseif yu == 2 then
                ...
                if sum > 0 and not self:split(cards, begin, i) then
                    return
                end
                t = {}
                begin = nil
...
function test_fail()
	...
	if not mj:get_hu_info(cards) then
